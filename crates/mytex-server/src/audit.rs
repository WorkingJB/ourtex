...
//!
//! Same shape as `mytex-audit` (seq, ts, actor, action, document_id,
//! scope_used, outcome, prev_hash, hash) so a future "export audit" job
//! can emit JSONL that `mytex-audit::verify` ingests unchanged. The only
...
/// `mytex_audit::Actor` with one additional variant.
...
/// identical to `mytex-audit`'s `HashInput` to keep both verifiers
/// compatible.
#[derive(Serialize)]
struct HashInput<'a> {
    seq: u64,
