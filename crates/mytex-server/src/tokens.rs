...
//!
//! Same token shape as `mytex-auth::StoredToken` (opaque `mtx_*` secret,
...
fn normalize_scope(raw: Vec<String>) -> Result<Vec<String>, ApiError> {
    ...
    // Validate each label via mytex_vault::Visibility. Keeps us honest
    // about what a scope label means: the literal string that must
    // match a document's `visibility`.
    for label in &raw {
        mytex_vault::Visibility::from_label(label).map_err(|_| {
